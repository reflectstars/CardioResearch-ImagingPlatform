...
/*=========================================================================
 *
 * Power transmitter Plugin for MITK
 *
...
void powertransViewPlot::SetDirectory(const QString directory) {
    powertransViewPlot::directory = directory;
}

void powertransViewPlot::SetNoFrames(int frames, int smoothness) {
    powertransViewPlot::noFrames = frames;
...
void powertransViewPlot::HandleCurvPlot() {
    ...
    QString msg = "Curves were not computed. Check the order of selected landmarks! For instance, order of two points on RV cusps.";
    if (nonComputable)
